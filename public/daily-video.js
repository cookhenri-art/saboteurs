...
class DailyVideoManager {
  // V41: Fonction de log conditionnelle
  static log(...args) {
    if (window.SABOTEUR_DEBUG) {
      DailyVideoManager.log('', ...args);
    }
  }
...
`;
    ...
    DailyVideoManager.log(" Container injected (v5.2-ui).");
  }
  ...
  async joinRoom(roomUrl, userName, permissions = { video: true, audio: true }) {
    ...
    DailyVideoManager.log(' joinRoom called, headless:', this.headless, 'isMobile:', this.isMobile);
    ...
    // ========================================
    // 🔥 OPTIMISATIONS MOBILE V2.1 - MOBILE ONLY
    // ========================================
    
    // Configuration qualité MOBILE uniquement (PC = défaut Daily)
    const mobileVideoConfig = {
      maxQuality: 'low',  // 180p
      encodings: {
        maxBitrate: 80000,     // 🔥 80 kbps (très bas)
        maxFramerate: 10       // 🔥 10 fps
      }
    };

    // PC : on ne touche à RIEN, paramètres par défaut Daily
    const videoConfig = this.isMobile ? mobileVideoConfig : null;
    
    DailyVideoManager.log(' 📊 Quality config:', this.isMobile ? 'MOBILE (180p/10fps/80kbps)' : 'DESKTOP (défaut Daily)');
    ...
    // JOIN standard (les paramètres de qualité sont appliqués APRÈS)
    ...
    // 🔥 OPTIMISATIONS MOBILE UNIQUEMENT (PC = rien)
    if (this.isMobile) {
      try {
        // Réduire la qualité d'envoi sur mobile
        if (videoConfig && typeof this.callFrame.updateSendSettings === 'function') {
          await this.callFrame.updateSendSettings({
            video: videoConfig
          });
          DailyVideoManager.log(' 📊 MOBILE send settings applied (80kbps/10fps)');
        }
        
        // Réduire la qualité de réception sur mobile (tous les joueurs en basse qualité)
        if (typeof this.callFrame.updateReceiveSettings === 'function') {
          await this.callFrame.updateReceiveSettings({
            '*': { video: { layer: 0 } }
          });
          DailyVideoManager.log(' 📊 MOBILE receive settings applied (layer 0 for all)');
        }
      } catch (e) {
        console.warn('[DailyUI] Mobile quality settings not applied:', e.message);
      }
    } else {
      DailyVideoManager.log(' 📊 DESKTOP: using Daily defaults (no changes)');
    }
    ...
    DailyVideoManager.log(" Joined room (v5 + MOBILE OPTIMIZED).");
    
    // V41: Déclencher video-tracks.js maintenant que le callObject existe
    if (typeof window.VideoTracksStartWaiting === 'function') {
      window.VideoTracksStartWaiting();
    }
  }

  // 🔥 MOBILE: Limiter le nombre de flux vidéo reçus pour économiser CPU/batterie
  setupMobileVideoLimit() {
    if (!this.callFrame || !this.isMobile) return;
    
    const maxStreams = this.maxVideoStreams || 4;
    DailyVideoManager.log(' 📊 Setting up mobile video limit:', maxStreams, 'streams max');
    
    // Écouter les changements de participants
    this.callFrame.on('participant-joined', () => this.enforceMobileVideoLimit());
    this.callFrame.on('participant-updated', () => this.enforceMobileVideoLimit());
    
    // Appliquer immédiatement
    this.enforceMobileVideoLimit();
  }
  
  // 🔥 Désactiver la réception vidéo des participants au-delà de la limite
  async enforceMobileVideoLimit() {
    if (!this.callFrame || !this.isMobile) return;
    ...
    try {
      ...
      if (remoteParticipants.length <= maxStreams) return; // Pas besoin de limiter
      
      // Garder les X premiers, désactiver les autres
      const toDisable = remoteParticipants.slice(maxStreams);
      
      for (const p of toDisable) {
        const sessionId = p.session_id || p.id;
        if (sessionId && typeof this.callFrame.updateReceiveSettings === 'function') {
          await this.callFrame.updateReceiveSettings({
            [sessionId]: { video: false }  // Désactiver complètement la vidéo
          });
        }
      }
      
      if (toDisable.length > 0) {
        DailyVideoManager.log(' 📊 Mobile: disabled video for', toDisable.length, 'participants (keeping', maxStreams, ')');
      }
    } catch (e) {
      console.warn('[DailyUI] enforceMobileVideoLimit error:', e.message);
    }
  }
...
DailyVideoManager.log(" dailyVideo manager ready ✅", window.dailyVideo.__version);