...
(function() {
  ...
  class VideoModeController {
    ...
    /**
     * Active speaker change (depuis Daily/LiveKit)
     * D5: Amélioration avec meilleur support tous modes
     * V4 FIX: Réduction du spam de logs pour éviter chauffe mobile
     */
    setActiveSpeaker(playerId) {
      // V4 FIX: Ignorer immédiatement si c'est le même speaker (avant même le debounce)
      if (this.activeSpeakerId === playerId) {
        return; // Pas de log, pas de timer, rien
      }
      
      // V4 FIX: Throttle - ignorer si on a reçu un événement il y a moins de 500ms
      const now = Date.now();
      if (this._lastSpeakerEventTime && (now - this._lastSpeakerEventTime) < 500) {
        return; // Trop rapide, ignorer
      }
      this._lastSpeakerEventTime = now;
      ...
      this.speakerDebounceTimer = setTimeout(() => {
        if (this.activeSpeakerId === playerId) {
          return; // Double check après debounce
        }
        
        const previousSpeaker = this.activeSpeakerId;
        this.activeSpeakerId = playerId;
        this.log('🎙️ Speaker changed:', playerId?.slice(0,8), '(was:', previousSpeaker?.slice(0,8), ')');
