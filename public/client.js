...
function setBackdrop() {
  ...
  if (p === "LOBBY" || p === "MANUAL_ROLE_PICK" || p === "ROLE_REVEAL" || p === "CAPTAIN_CANDIDACY" || p === "CAPTAIN_VOTE" || p === "CAPTAIN_RESULT") {
...
if (state.phase === "CAPTAIN_CANDIDACY") {
  ...
  if (state.phase === "CAPTAIN_VOTE" || state.phase === "CAPTAIN_RESULT") {
...
function renderEnd() {
  ...
  if (state.phase === "GAME_ABORTED") {
    ...
    const draw = window.i18n ? window.i18n('game.endGame.draw') : "⚖️ ÉGALITÉ";
    
    // V42: Gestion de l'égalité (Security revenge)
    let titleText;
    if (winner === "SABOTEURS") {
      titleText = `${victoryOf} ${t('saboteurs').toUpperCase()}`;
    } else if (winner === "AMOUREUX") {
      titleText = association;
    } else if (winner === "EGALITE") {
      titleText = draw;
    } else {
      titleText = `${victoryOf} ${t('astronauts').toUpperCase()}`;
    }
...
function buildPhaseText(s) {
  ...
  if (p === "CAPTAIN_VOTE" || p === "CAPTAIN_RESULT") return tr('game.phaseDesc.captainVote', `Vote pour élire le ${t('captain').toLowerCase()}. En cas d'égalité : revote.`);
...
class AudioManager {
  ...
  unlock() {
    // PERF FIX: Ne rien faire si déjà unlocked et pas de cue en attente
    if (this.userUnlocked && !this.pendingCue) {
      return; // Déjà déverrouillé et rien à jouer, ignorer
    }
  ...
  tts(text) {
    ...
    // V42: Protection robuste contre les doublons
    const now = Date.now();
    if (this._lastTtsText === text && this._ttsStartTime && (now - this._ttsStartTime < 8000)) {
      console.log('[audio] TTS skip duplicate:', text.substring(0, 30));
      return;
    }
    
    // V42: Annuler tout TTS en attente
    if (this._ttsTimeout) {
    ...
    const fixedTranslations = {
      ...
      '{{captainTransfer}}': {
        ...
        nl: 'Kapiteinoverdracht.'
      }
    };
    
    // V42: Traduction pour résultat élection capitaine (avec nom)
    const captainResultMatch = text.match(/\{\{captainResult:(.+?)\}\}/);
    if (captainResultMatch) {
      const captainName = captainResultMatch[1];
      const templates = {
        fr: `${captainName} est élu capitaine.`,
        en: `${captainName} has been elected captain.`,
        es: `${captainName} ha sido elegido capitán.`,
        de: `${captainName} wurde zum Kapitän gewählt.`,
        it: `${captainName} è stato eletto capitano.`,
        pt: `${captainName} foi eleito capitão.`,
        nl: `${captainName} is verkozen tot kapitein.`
      };
      translatedText = templates[lang] || templates['fr'];
    }
    ...
    try {
      ...
      // V42: Délai aléatoire (0-400ms) + délai fixe pour éviter conflits multi-onglets sur même PC
      const randomDelay = Math.floor(Math.random() * 400);
      this._ttsTimeout = setTimeout(() => {
        this._ttsTimeout = null;
        try {
          // Cancel uniquement si quelque chose est vraiment en cours ET différent
          if ((window.speechSynthesis.speaking || window.speechSynthesis.pending) && this._lastSpokenText !== translatedText) {
            console.log('[audio] TTS cancel previous (different text)');
            try { window.speechSynthesis.cancel(); } catch {}
          }
          
          this._lastSpokenText = translatedText;
          const u = new SpeechSynthesisUtterance(translatedText);
          u.lang = langMap[lang] || 'fr-FR';
          
          console.log('[audio] TTS speaking:', translatedText.substring(0, 50), '(lang:', u.lang, ', delay:', randomDelay + 300, 'ms)');
      ...
      }, 300 + randomDelay); // V42: Délai fixe + aléatoire pour tests multi-onglets
    } catch (err) {
      console.warn("[audio] TTS error:", err);
    }
  }
...
socket.on("roomState", (s) => {
  ...
  // V42: Stocker roomCode et playerToken dans sessionStorage pour les stats postgame
  if (s.roomCode) {
    sessionStorage.setItem('saboteur_room', s.roomCode);
    // Récupérer le playerToken depuis localStorage et le stocker aussi dans sessionStorage
    const pt = localStorage.getItem('is_playerToken');
    if (pt) {
      sessionStorage.setItem('saboteur_playerToken', pt);
    }
  }
...
function tRole(roleKey, plural = false) {
  // V41: Utiliser homeSelectedTheme (localStorage) comme fallback au lieu de 'default'
  const themeId = currentTheme?.id || homeSelectedTheme || 'default';
  const lang = window.getCurrentLanguage ? window.getCurrentLanguage() : 'fr';
 ...
 */
let _lastAppliedTheme = null; // PERF FIX: Cache du dernier thème appliqué
function applyThemeStyles(themeId) {
  // PERF FIX: Ne pas ré-appliquer si même thème
  if (_lastAppliedTheme === themeId) {
    return;
  }
...
// =====================================================
// PERF V10: Log supprimé (version info)
