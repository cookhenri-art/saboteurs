...
// PERF FIX: Throttle pour éviter les appels répétés
let _lastTranslateTime = 0;
let _lastTranslateLang = null;
const TRANSLATE_THROTTLE_MS = 500; // Minimum 500ms entre les traductions
...
function translateGamePage() {
  ...
  // PERF FIX: Throttle - ignorer si appelé trop souvent avec la même langue
  const now = Date.now();
  if (_lastTranslateLang === lang && (now - _lastTranslateTime) < TRANSLATE_THROTTLE_MS) {
    return; // Déjà traduit récemment, ignorer
  }
  _lastTranslateTime = now;
  _lastTranslateLang = lang;
